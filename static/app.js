...
function modelToolbarHtml(models){const list=normalizeModels(models);const enabled=list.filter(m=>m.enabled).length;return `<div class="model-toolbar"><span class="pill">共 ${list.length} 个模型</span><span class="pill">启用 ${enabled}</span><span class="pill">启用即支持参考图</span></div>`}
function modelFetchState(models, existingModels, defaultModel, providerSupportsReference){
  ...
  const merged = normalizeModels(models).map(model=>{
    ...
    return current ? {...model, enabled:current.enabled, supports_reference:current.enabled || current.supports_reference, name:current.name||model.name} : {...model, enabled:false, supports_reference:false};
  });
...
function modelFetchModalHtml(state){
  ...
  return `<div class="modal model-fetch-modal open" id="modelFetchModal"><div class="modal-card"><div class="modal-head"><div><h3>拉取模型</h3><p class="small">勾选需要启用的模型；已启用模型会自动支持参考图。</p></div><button class="btn light" type="button" id="closeModelFetchModal">关闭</button></div><div class="modal-body">${!list.length?'<div class="model-empty">没有拉取到任何模型。</div>':`<div class="modal-actions inline"><button class="btn light" type="button" id="selectAllFetchedModels">全选</button><button class="btn light" type="button" id="clearFetchedModels">清空</button></div><div class="fetched-model-list">${list.map(model=>`<div class="fetched-model-item" data-fetched-model-row data-model-id="${escapeHtml(model.id)}"><div class="model-main"><div class="model-title"><b>${escapeHtml(model.name)}</b><span>${escapeHtml(model.id)}</span></div><div class="model-badges">${model.enabled?'<span class="badge ok">已选择</span>':''}${state.defaultModel===model.id?'<span class="badge accent">当前默认</span>':''}${model.enabled?'<span class="badge warm">自动支持参考图</span>':''}</div></div><div class="model-controls"><label class="model-toggle"><input type="checkbox" data-fetched-enabled ${model.enabled?'checked':''}> 启用</label><label class="model-toggle"><input type="radio" name="fetched_default_model" value="${escapeHtml(model.id)}" ${state.defaultModel===model.id?'checked':''} ${model.enabled?'':'disabled'}> 默认调用</label></div></div>`).join('')}</div>`}</div><div class="modal-actions"><button class="btn light" type="button" id="cancelModelFetchModal">取消</button><button class="btn primary" type="button" id="applyFetchedModels">应用选择</button></div></div></div>`;
}
...
function bindModelFetchModal(){
  ...
  $('#applyFetchedModels').onclick = ()=>{
    const rows = $$('[data-fetched-model-row]', modal);
    const selected = rows.map(row=>({id:row.dataset.modelId,name:$('.model-title b', row).textContent,enabled:$('[data-fetched-enabled]', row).checked,supports_reference:$('[data-fetched-enabled]', row).checked}));
...
function bindModelPicker(){
  ...
  const sync = (rerender=true)=>{
    ...
    const models = rows.map(row=>{const enabled=$('[data-model-enabled]',row).checked; return {id:row.dataset.modelId,name:row.dataset.modelName,enabled,supports_reference:enabled};});
